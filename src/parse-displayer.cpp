...
namespace word_stacker
{
    ...
    :
        m_uniqueColors(sf::Color(sf::Uint8(100), sf::Uint8(100), sf::Uint8(255)),
        ...
        m_willShowLineLengthGraph(false)
    {
        ...
        ColorRangeVec_t colorRanges = {
            ColorRange(sf::Color(160, 82, 45),//brown
                       sf::Color(255,165,0),//orange
                       M_GRADIENT_DEPTH * 4),
            ColorRange(sf::Color(255,165,0),//orange
                       sf::Color::Yellow,
                       M_GRADIENT_DEPTH * 2),
            ColorRange(sf::Color::Yellow,
                       sf::Color::White,
                       M_GRADIENT_DEPTH / 32) };
        ...
        const bool         WILL_SHOW_LINELEN_GRAPH)
    {
        ...
        WordCountVec_t wordCounts;
        {
            ...
            M_LOG_AND_ASSERT_OR_THROW((PARSED_WORDS.empty() == false),
        ...
        auto const STATS{ Statistics::calculate(
            wordCounts,
            "Displayed Frequency List:\t",
            reportMaker.frequencyListLength()) };
        ...
        const unsigned         FONT_SIZE_MAX)
    {
        ...
        auto const FREQ_RATIO{ (((WORDS.size() == 1) || ((FREQ - FREQ_MIN) == 0)) ?
                1.0f :
        ...
        auto const COMMON_RATIO{
            (COMMON_WORDS_COUNT_F - static_cast<float>(COMMON_ORDER)) / COMMON_WORDS_COUNT_F };
        ...
        auto const FONT_SIZE{ FONT_SIZE_MIN + static_cast<unsigned>(
            static_cast<float>(FONT_SIZE_MAX - FONT_SIZE_MIN) * FREQ_RATIO) };

        sf::Text wordText(WORD, m_font, FONT_SIZE);

        wordText.setColor( ((COMMON_ORDER == 0) ?
            m_uniqueColors.colorAtRatio(FREQ_RATIO) :
        ...
        if (WILL_SHOW_COUNTS)
        {
            ...
            auto const COUNT_TEXT_VERT_POS{
                ((wordText.getGlobalBounds().height * 0.5f) -
                 (countText.getGlobalBounds().height * 0.5f)) + EXTRA_HEIGHT };
        ...
        const sf::Vector2f SIZE{
            ...
            std::max(
                wordText.getGlobalBounds().height,
                countText.getGlobalBounds().height) + (1.5f * EXTRA_HEIGHT) };
    ...
    void ParseDisplayer::loadFont(const ArgsParser & ARGS)
    {
        ...
        M_LOG_AND_ASSERT_OR_THROW((m_font.loadFromFile(PATH_STR.c_str())),
            "During ParseDisplayer::LoadFont() sf::Font::loadFromFile(\"" << PATH_STR
        ...
        const unsigned         FONT_SIZE_MAX)
    {
        ...
        while (wordsIndex < WORDS.size())
        {
            ...
            //center the count text to the left of the wordText
            auto const COUNT_TEXT_VERT_POS{
                (vertPos + (wordText.getGlobalBounds().height * 0.5f)) -
                    (countText.getGlobalBounds().height * 0.5f) - VERT_SHIFT };
            ...
            auto const HORIZ_EXTENT{
                wordText.getGlobalBounds().left + wordText.getGlobalBounds().width };
            ...
            auto const VERT_SPACE_CONSUMED{
                std::max(countText.getGlobalBounds().height, wordText.getGlobalBounds().height) };
        ...
        sf::RenderStates states) const
    {
        ...
        auto const RAW_RECT_WIDTH{
            (GRAPH_WIDTH / static_cast<float>(LINELEN_MAP.size())) *
        ...
        lineVerts.push_back(sf::Vertex(
            sf::Vector2f(GRAPH_LEFT, GRAPH_TOP), m_uniqueColors.colorAtRatio(1.0f)));

        lineVerts.push_back(sf::Vertex(
            sf::Vector2f(GRAPH_LEFT, GRAPH_BOTTOM), m_uniqueColors.colorAtRatio(0.5f)));

        lineVerts.push_back(sf::Vertex(
            sf::Vector2f(GRAPH_LEFT, GRAPH_BOTTOM), m_uniqueColors.colorAtRatio(1.0f)));
        ...
        for (auto const & LENGTH_COUNT_PAIR : LINELEN_MAP)
        {
            auto const LINE_LENGTH_RATIO{
                static_cast<float>(LENGTH_COUNT_PAIR.first) /
                    static_cast<float>(lineLengthMax) };

            auto const LINE_COUNT_RATIO{
                static_cast<float>(LENGTH_COUNT_PAIR.second) /
                    static_cast<float>(lineCountMax) };

            auto const LEFT{
                (GRAPH_LEFT + (GRAPH_WIDTH * LINE_LENGTH_RATIO)) };
