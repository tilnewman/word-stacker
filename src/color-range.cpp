...
namespace word_stacker
{
        ...
        : m_colors()
    {
        for (auto const & COLOR_RANGE : COLOR_RANGE_VEC)
        {
            ...
            std::copy(SUB_COLOR_RANGE.begin(), SUB_COLOR_RANGE.end(), std::back_inserter(m_colors));
        }
    }
        ...
        const sf::Color & FIRST_COLOR, const sf::Color & LAST_COLOR, const std::size_t LENGTH)
    {
        ...
        auto redStep{ (static_cast<float>(LAST_COLOR.r) - red) / static_cast<float>(LENGTH) };

        auto greenStep{ (static_cast<float>(LAST_COLOR.g) - green) / static_cast<float>(LENGTH) };

        auto blueStep{ (static_cast<float>(LAST_COLOR.b) - blue) / static_cast<float>(LENGTH) };

        auto alphaStep{ (static_cast<float>(LAST_COLOR.a) - alpha) / static_cast<float>(LENGTH) };
    ...
    void ColorRange::Draw(
        sf::RenderTarget & target, sf::RenderStates states, sf::FloatRect rect) const
    {
        M_LOG_AND_ASSERT_OR_THROW(
            (rect.width > 1.5f),
            "word-stacker::ColorRange::Draw(rect.width=" << rect.width
                                                         << ") rect.width was less than 2.0f.");
        ...
        for (int i(0); i < static_cast<int>(rect.width); ++i)
        {
            ...
            verts.push_back(sf::Vertex(sf::Vector2f(rect.left + FLOAT_I, rect.top), COLOR));

            verts.push_back(
                sf::Vertex(sf::Vector2f(rect.left + FLOAT_I, rect.top + rect.height), COLOR));
        }
    ...
    {
        M_LOG_AND_ASSERT_OR_THROW(
            (m_colors.empty() == false),
            "word-stacker::ColorRange::colorAtRatio(ratio=" << RATIO_PARAM
                                                            << ") m_colors vector was empty.");

        auto const MAX_INDEX{ static_cast<int>(m_colors.size() - 1) };

        auto index{ static_cast<int>(RATIO_PARAM * static_cast<float>(MAX_INDEX)) };
